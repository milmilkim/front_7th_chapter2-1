const RelatedProducts = ({ products, currentProductId }) => {
  ...
  const filteredProducts = products.filter((p) => p.productId !== currentProductId);
        ...
        <div class="grid grid-cols-2 gap-3 responsive-grid">
          ${filteredProducts
            ...
            <div class="bg-gray-50 rounded-lg p-3 related-product-card cursor-pointer" data-product-id="${product.productId}">
              ...
              <p class="text-sm font-bold text-blue-600">${Number(product.price ?? product.lprice ?? 0).toLocaleString()}원</p>
            </div>
          `,
            )
            .join("")}
