      ...
      <div class="p-4">
        <div class="grid grid-cols-2 gap-3 responsive-grid">
          <div class="bg-gray-50 rounded-lg p-3 related-product-card cursor-pointer" data-product-id="86940857379">
            ...
            <h3 class="text-sm font-medium text-gray-900 mb-1 line-clamp-2"판 창문 바람막이 베란다 문 틈막이 창틀 벌레 차단 샤시 방충망 틈새막이</h3>
...
const DetailPage = async ({ root, params }) => {
  const productId = params?.productId;
  let isLoading = false;
  let product = {};

  const getTemplate = () => /*html*/ `
    ${isLoading ? pageLoading() : ProductInfo(product)}
  `;
